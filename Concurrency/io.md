...
- 中断当前进程，陷入内核进程
- 内核直接注册 "epoll_fd" 挂载到 epoll 的红黑树中
  - 用户态没法注册
...
- 需要接收方重写并注册对应信号的处理方法，可以跨进程组、跨用户发送（有权限即可）
- 是单工通信
...
- (Unix only) 是半双工的，读写无法同时进行。但允许远程网络进程通讯
...
|     Named Pipe     |   支持   | 在 Unix 上可以通过 `fs.mkfifo` + 文件流操作；Windows 上可通过 `\\.\pipe\` 路径 + net 模块或 named-pipe 库 |
