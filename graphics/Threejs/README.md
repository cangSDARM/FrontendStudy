...
```html
//Threejs使用的Shader语言是GLSL /*创建自定义的着色器，需要实现两个组件:
vertexShader和fragmentShader 组件vertexShader 用来调整每个顶点的位置;
组件fragmentShader可以从来决定每个像素的颜色
对于后期处理着色器来说，我们只要实现fragmentShader即可,
然后使用Three.js提供的默认vertexShader */ //1. 在html里写Shader
...
//2. 在js里写Shader THREE.CustomGrayScaleShader = { uniforms: { "tDiffuse":
{type: "t", value: null}, "rPower": {type: "f", value: 0.2}, "gPower": {type:
"f", value: 0.7}, "bPower": {type: "f", value: 0.1} }, vertexShader: ` varying
vec2 vUv;, void main(){, vUv = uv;, gl_Position = projectionMatrix *
modelViewMatrix * vec4(position, 1.0);, } `, fragmentShader: ` uniform float
rPower;, uniform float gPower;, uniform float bPower;, uniform sampler2D
tDiffuse;, varying vec2 vUv;, void main(){, vec4 texel = texture2D(tDiffuse,
vUv);, float gray = texel.r * rPower + texel.g * gPower + texel.b * bPower;,
gl_FragColor = vec4(vec3(gray), texel.a);, } ` }
```
