...
2. the decisions of the problem are reversible? if one of them is been chosen, how costly to change to another?
   ...
   2. irreversible. decide as late as possible, collect infos as many as possible.
3. define the evaluation criteria, make different decisions can be quantified
   1. what factors can influence and them relative importance?
   2. what constraints exists? eg: Cost cannot exceed $xx
4. (Optional in some case) select the participants. decisions cannot make by yourself only
   1. Who should be consulted before making a decision?
   2. Does anyone need to approve the decision? Ask them, to understand their needs and incentives
   3. Who should be informed of the final decision?
   4. Who will execute the decision?
5. later, retrospect
   1. problem actually solved?
   2. making process is thoughtfully or efficiently?
   3. what can we learn?)

Open Source Business Modals^(
(1) open core, offering a “core” or feature-limited version of a product as Open Source software like CE EE modal
(2) hosting (a cloud service), use more careful licensing format may help for competition from AWS Azure GCP
...
(2) the user can tell what is going on. for example submit-loading: not alert all the time or no alert, wait least 0.2s to show loading(because 0.1s means there is no loading at all)
) https://blog.encodeart.dev/the-only-two-ux-rules-you-need-to-know
