...
```js
class FreeList {
  constructor(Constructor, maxSize = 10) {
  ...
  acquire(...args) {
    return this.list.length > 0 ? this.list.pop() : Reflect.construct(this.Constructor, args);
  }

  release(instance) {
    if (!(instance instanceof this.Constructor)) throw new Error('Unmatched instance');
...
> 解析
>
> 仅 OSX/Android Chrome 支持，否则依然需要库
...
// node
const compile = (code) => {
  const module = new Module("xyz");
  module._compile(code, "xyz");
...
class Module {
  #exports = {};
  #wrap(script) {
    return `return (function(exports, module) { ${script} });`;
  }
  ...
  compile(code) {
    const wrapper = this.#wrap(code);
    this.#runInContext(wrapper)({}, ["console", "window"]).call(
...
const module = new Module().compile(`
module.exports = {
  name: 'ok',
  action: () => console.log(this.name);
}
`);
module.action();
```
