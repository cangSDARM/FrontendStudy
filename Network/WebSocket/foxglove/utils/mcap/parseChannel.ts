...
function parsedDefinitionsToDatatypes(parsedDefinitions: MessageDefinition[], rootName?: string): MessageDefinitionMap {
 ...
 * channel, and schemas in the format expected by RosDatatypes.
 *
 ...
 */
export function parseChannel(channel: Channel, options?: { allowEmptySchema: boolean }): ParsedChannel {
  ...
  if (channel.messageEncoding === "json") {
    ...
    if (channel.schema != undefined) {
      const schema = channel.schema.data.length > 0 ? JSON.parse(textDecoder.decode(channel.schema.data)) : undefined;
      if (schema != undefined) {
        ...
        deserialize = data => postprocessValue(JSON.parse(textDecoder.decode(data)) as Record<string, unknown>);
      }
    }
    ...
    if (channel.schema?.encoding !== "flatbuffer") {
      throw new Error(
        `Message encoding ${channel.messageEncoding} with ${
          channel.schema == undefined ? "no encoding" : `schema encoding '${channel.schema.encoding}'`
        } is not supported (expected flatbuffer)`
      );
    }
    log.error("flatbuffer dont support yet");
  ...
  if (channel.messageEncoding === "cdr") {
    ...
    if (channel.schema.encoding === "omgidl") {
      throw new Error("omgidl don't support yet", { cause: channel });
      ...
      const parsedDefinitions = isIdl ? parseRos2idl(schema) : parseMessageDefinition(schema, { ros2: true });
