...
export function parseJsonSchema(
  ...
  function addFieldsRecursive(
    ...
    if (schema.type !== "object") {
      throw new Error(`Expected "type": "object" for schema ${typeName}, got ${JSON.stringify(schema.type)}`);
    }
    ...
    for (const [fieldName, fieldSchema] of Object.entries(properties)) {
      ...
      switch (fieldSchema.type) {
              ...
              postprocessObject = value => {
                ...
                if (typeof str === "string") {
                  ...
                  if (base64.decode(str, decoded, 0) !== decoded.byteLength) {
                    throw new Error(`Failed to decode base64 data for ${keyPath.concat(fieldName).join(".")}`);
                  }
        ...
        case "integer":
          fields.push({
            ...
            type:
              ...
              (typeof fieldSchema.exclusiveMinimum === "number" && fieldSchema.exclusiveMinimum >= 0)
                ? "uint32"
                : "int32",
          });
          break;
        case "object": {
          ...
          const postprocessNestedObject = addFieldsRecursive(fieldSchema, nestedTypeName, keyPath.concat(fieldName));
          const prevPostprocess = postprocessObject;
          postprocessObject = value => {
            ...
            case "integer":
              fields.push({
                ...
                type:
                  ...
                  (typeof itemSchema.exclusiveMinimum === "number" && itemSchema.exclusiveMinimum >= 0)
                    ? "uint32"
                    : "int32",
                isArray: true,
              });
