- [Unicode 字符集](#unicode-字符集)
  - [元字符集](#元字符集)
    ...
    - [类别](#类别)
    - [字素 \<-\> 元字符集](#字素---元字符集)
  - [码位 Code Point](#码位-code-point)
  - [码元 Code Unit](#码元-code-unit)
    - [UTF-32](#utf-32)
    - [UTF-8](#utf-8)
    - [UTF-16](#utf-16)
    - [UCS-2](#ucs-2)
...
|        | Character | Code Point | Code Unit |
| :----: | :-------: | :--------: | :-------: |
| UTF-8  |     ก     |   0x0E01   | E8 B8 B1  |
| UTF-16 |     ก     |   0x0E01   |   0E 01   |
...
但做为(字素的)元数据，区域设置通常没有被(Unicode/字体作为元数据)保留/统一，因此经常出现映射的字体表现不一的情况
...
1. NFC，假设可以 1:1 映射，如果不行在查找可能的元字符组合
2. NFD，假设字素都可分解为元字符，如果不行再 1:1 映射
...
- UTF-16 中代理对可以表达的内容是由平面数(17)决定的(因为标准和 UTF-16 的编码规则是同时同一批人制定的)

码位写法：`U+1F4A9`

### 码元 Code Unit

即是各种编码映射码位后的具体占位字节内容
...
UTF-8 的变长设计导致同一个码元可以被编码为多种不同长度的序列，如 U+002F (/) 有 `0x2F`/`0xC0 0xAF`/`0xE0 0x80 0xAF`。
会导致验证字符过于复杂的非一致性。现在根据 Unicode 标准，合法的 UTF-8 编码必须使用最短形式，过长序列应被视为错误
...
- 如果遇到的字节的最高位是 1，那么该字符可能两字节，也可能四字节：
  - 下一字节是 0x30-0x39，那么该字符四字节长
  - 下一字节是 0x40~0xFE，那么该字符两字节长
...
"的".codePointAt(0); // 返回字符串对应位置的 Unicode 编号, 能正确处理四字节或两字节
...
// 遇到代理区内的单个码元，或者不存在的配对形式，它会返回转义字符串
JSON.stringify("\uDF06\uD834"); // "\\udf06\\ud834"
```

string 和 ArrayBuffer 间转换
...
/** display arraybuffer like hexadecimal */
function arrayBuffer2Hex(arraybuffer: ArrayBuffer) {
